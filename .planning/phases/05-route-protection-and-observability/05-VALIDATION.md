...
| 05-02-02 | 02 | 0 | OBSV-01 | integration | `npx vitest run tests/observability.test.ts -t "Tool invocation logging"` | W0 | pending |
...
- [ ] `tests/route-protection.test.ts` -- stubs for PROT-01, PROT-02, PROT-03, PROT-04 using Fastify inject() with mocked auth middleware (created by Plan 02 Task 2)
- [ ] `tests/observability.test.ts` -- stubs for OBSV-01 (tool invocation logging with toolName/duration/userId/username), OBSV-02 (correlation ID propagation) using Fastify inject() with pino destination capture (created by Plan 02 Task 3)
- [ ] `tests/auth-errors.test.ts` -- stubs for OBSV-03 unit testing jose error -> RFC 6750 mapping function (created by Plan 01 Task 1)
